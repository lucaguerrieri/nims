...
total_assets = sum(rochelle_data(:,1:nassets),2);
...
plot([nim_loans_commercial nim_loans_consumer nim_loans_other nim_loans_realestate nim_securities_notrading nim_securities_trading])
...
this_str =num2str(this_asset);
eval(['[logLikel,errcode,xi10History,xi1tHistory_baseline_',this_str,',errHistory]',...
      '= kalmanFilterSmoother(cofcompanion_',this_str,', transpose(h), transpose(y_baseline),',...
      ' xi10_',this_str,', P10, q_',this_str,', ntrain);'])

end
 
xi1tHistory_baseline = 0*xi1tHistory_baseline_1;
for this_asset = 1:nassets
this_str =num2str(this_asset);
   eval([' xi1tHistory_baseline = xi1tHistory_baseline+',deblank(rochelle_varlist(this_asset,:)),'_share(end)*xi1tHistory_baseline_',this_str,';']);
end
...
eval(['[replication_',this_str,'] = confint_partial(cofb_',this_str,',const_',this_str,',errmat_',this_str,',var_data_',this_str,'(1:end-14,:),varlag,nreps,isconstant,iscontemp,yw,h,y_baseline,xi10_',this_str,',P10,q_',this_str,',ntrain,errpos)']);
...
confint_lb_baseline = replication(5,:);
confint_ub_baseline = replication(95,:);
