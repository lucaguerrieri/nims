...
rochelle_varlist = char('assets_securities_notrading','assets_securities_trading','assets_loans_realestate','assets_loans_commercial','assets_loans_consumer','assets_loans_other',...
'revenue_securities_notrading','revenue_securities_trading','revenue_loans_realestate','revenue_loans_commercial','revenue_loans_consumer','revenue_loans_other','expenses_total');

rochelle_data = xlsread('interestbearing_assets_revenues_expenses.xls');

rochelle_dates = 1985:.25:2012;
...
% stop estimation sample in 2008Q3 to avoid ZLB period.
%endpos = find(var_dates==2008.5)-9;


% I am using the Yule-Walker conditions to estimate the VAR instead of OLS
% The Yule-Walker guarantees stationarity of the VAR
% For references check Lutkepohl "Introduction to Multiple Time Series
% Analysis", page 78.
%[A,a0,coverr,errmat] = ywestimate(var_data(1:end-14,:),4); 
%[cofb,const]=yw2ar(A,a0); 
...
isconstant = 1;
iscontemp = 0;
[cofb,const]=ols2ar(coefs(1:end,:),isconstant,iscontemp);
 

% Express the VAR in companion form
cofcompanion = companion(cofb,const);
...
% h is the matrix in the observation equation
h = zeros(1,size(cofcompanion,1));
...
%xi10 contains the lagged state in the state equation
%xi10a = [var_data(end,:) var_data(end-1,:) var_data(end-2,:) var_data(end-3,:) 1]';

%1 is for the constant
xi10 = [var_data(end,:)]';

for lag_num=1:varlag-1
    xi10 = [xi10; var_data(end-lag_num,:)'];
end;    

xi10 = [xi10; 1];

% y is the data vector -- in this case it contains the conditioning
% information on the path of RTB starting in Q2
...
%ymat = [y [xi10(5);y(1:end-1)] [xi10([10,5]);y(1:end-2)] [xi10([15,10,5]);y(1:end-3)]]; 

%NB:  q is the Cholesky of Q in Hamilton's notation
q = zeros(length(xi10));  %q = zeros(nvars*varlag+1);
q(1:nvars,1:nvars) = chol(coverr)';
...
P10 = 0.000000000001*eye(size(q,1));


% xi1tHistory contains the smoothed estimates
[logLikel,errcode,xi10History,xi1tHistory,errHistory] = kalmanFilterSmoother(cofcompanion, h', y', xi10, P10, q, ntrain);
...
[confint_lb_baseline confint_ub_baseline] = ...
    confint_symmetric(90,cofb,const,errmat,var_data(1:end-14,:),varlag,nreps,isconstant,...
            iscontemp,yw,h,y_baseline,xi10,P10,q,ntrain);
...
[confint_lb_baseline confint_ub_baseline] = ...
    confint_symmetric(cofb,const,errmat,var_data(1:end-14,:),varlag,nreps,isconstant,...
            iscontemp,yw,h,y_baseline,xi10,P10,q,ntrain,xi1tHistory_baseline(1,:));

[confint_lb_bear_flattener confint_ub_bear_flattener] = ...
    confint_symmetric(cofb,const,errmat,var_data(1:end-14,:),varlag,nreps,isconstant,...
            iscontemp,yw,h,y_bear_flattener,xi10,P10,q,ntrain,xi1tHistory_bear_flattener(1,:));


[confint_lb_bear_parallel confint_ub_bear_parallel] = ...
    confint_symmetric(cofb,const,errmat,var_data(1:end-14,:),varlag,nreps,isconstant,...
                         iscontemp,yw,h,y_bear_parallel,xi10,P10,q,ntrain,xi1tHistory_bear_parallel(1,:));
                     
[confint_lb_bear_steepener confint_ub_bear_steepener] = ...
    confint_symmetric(cofb,const,errmat,var_data(1:end-14,:),varlag,nreps,isconstant,...
                         iscontemp,yw,h,y_bear_steepener,xi10,P10,q,ntrain,xi1tHistory_bear_steepener(1,:));


[confint_lb_bull_flattener confint_ub_bull_flattener] = ...
    confint_symmetric(cofb,const,errmat,var_data(1:end-14,:),varlag,nreps,isconstant,...
                         iscontemp,yw,h,y_bull_flattener,xi10,P10,q,ntrain,xi1tHistory_bull_flattener(1,:));
end      
%%
figure
subplot(2,2,1)
