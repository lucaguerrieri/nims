...
for this_rep = 1:nreps
    % to block bootstrap across components, keep the replications constant across models
    ...
    if ~isempty(strmatch('baseline',scenario_str,'exact'))
       replication_baseline = replication; 
    else
       replication_difference = replication-replication_baseline;
    end
...
if ~symmetric_confint_switch
    ...
    if isempty(strmatch('baseline',scenario_str,'exact'))
        replication_difference = replication-replication_baseline;
        eval(['confint_lb_difference_',scenario_str,' = replication_difference(round(nreps/100*5),:);'])
        eval(['confint_ub_difference_',scenario_str,' = replication_difference(round(nreps/100*95),:);'])
    ...
    eval(['confint_lb_',scenario_str,' = xi1tHistory_',scenario_str,'(1,:)-1*std_vec;'])
    eval(['confint_ub_',scenario_str,' = xi1tHistory_',scenario_str,'(1,:)+1*std_vec;'])
    if isempty(strmatch('baseline',scenario_str,'exact'))
        replication_difference = replication-replication_baseline;
        std_vec =std(replication_difference);
        eval(['confint_lb_difference_',scenario_str,' = xi1tHistory_',scenario_str,'(1,:)-xi1tHistory_baseline(1,:)-1*std_vec;'])
        eval(['confint_ub_difference_',scenario_str,' = xi1tHistory_',scenario_str,'(1,:)-xi1tHistory_baseline(1,:)+1*std_vec;'])
...
figure
subplot(2,2,1)
plot(xi1tHistory_bear_flattener(1,:)-xi1tHistory_baseline(1,:),'k')
hold on
plot(confint_lb_difference_bear_flattener,'b--')
plot(confint_ub_difference_bear_flattener,'b--')
title('Bear Flattener')

subplot(2,2,2)
plot(xi1tHistory_bear_parallel(1,:)-xi1tHistory_baseline(1,:),'k')
hold on
plot(confint_lb_difference_bear_parallel,'b--')
plot(confint_ub_difference_bear_parallel,'b--')
title('Bear Parallel')

subplot(2,2,3)
plot(xi1tHistory_bear_steepener(1,:)-xi1tHistory_baseline(1,:),'k')
hold on
plot(confint_lb_difference_bear_steepener,'b--')
plot(confint_ub_difference_bear_steepener,'b--')
title('Bear Steepener')

subplot(2,2,4)
plot(xi1tHistory_bull_flattener(1,:)-xi1tHistory_baseline(1,:),'k')
hold on
plot(confint_lb_difference_bull_flattener,'b--')
plot(confint_ub_difference_bull_flattener,'b--')
title('Bull Flattener')
