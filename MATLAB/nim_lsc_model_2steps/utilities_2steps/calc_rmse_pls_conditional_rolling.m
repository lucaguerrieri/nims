function [rmse_mat, forecast_mat,forecast_errors_mat,insample_forecast_errors_mat] = calc_rmse_pls_conditional_rolling(yobs, other_obs, out_of_sample_start_pos, end_sample_pos, forecast_horizon, npls )

% FOR NOW -- reglag does not affect anything. The regression is run with
% only one lag of other_obs

%This function computes rmse, forecasts, and forecast errors using partial
% least squares.
%
%Inputs: 
...
%observation.
%npls is the number of the pls factors to be used.
%
%Outputs:
...
for pos_index = out_of_sample_start_pos:end_sample_pos
    ...
    this_u_x = (this_other_obs(:,end-1)'-mean_this_other_obs')-(this_yobs(end-1)-mean_this_yobs)*beta_x;
    insample_forecast = [this_yobs(end-1)-mean_this_yobs, this_u_x*R ]*...
                              beta_pls+mean_this_yobs;
