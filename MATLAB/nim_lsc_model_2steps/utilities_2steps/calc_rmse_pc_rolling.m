function [rmse_mat, forecast_mat, forecast_errors_mat, insample_forecast_errors_mat]...
    = calc_rmse_pc_rolling(yobs, other_obs, out_of_sample_start_pos, ...
                           end_sample_pos, forecast_horizon, npc )
...
for pos_index = out_of_sample_start_pos:end_sample_pos
    ...
    for this_step = 1:this_forecast_horizon
        ...
        forecast(this_step) = [1, previous_forecast, transpose(other_obs_out_of_sample(:,this_step))*V ]*beta_pc;
        previous_forecast = forecast(this_step);
    end
    
    insample_forecast = [1, this_yobs(end-1), transpose(this_other_obs(:,end-1))*V ]*beta_pc;
