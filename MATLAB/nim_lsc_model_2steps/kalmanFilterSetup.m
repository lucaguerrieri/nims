...
r_start = q_end+1;
r_end = q_end + ((ntaus+nothers)*((ntaus+nothers)+1))/2;
r_all = reshape(param_vec(r_start:r_end), ((ntaus+nothers)*((ntaus+nothers)+1)/2), 1);

%write for loop that automates this based on ntaus

r = zeros(ntaus+nothers);

rdiag_start_counter = 1;
rdiag_end_counter = 1;
for diag_index = (ntaus+nothers)-1:-1:1
    r=r+diag(r_all(rdiag_start_counter:rdiag_end_counter),diag_index);
    rdiag_start_counter=rdiag_end_counter+1;
    rdiag_end_counter = rdiag_end_counter+1+(ntaus+nothers)-diag_index;
end
%r = (diag(r_all(1), 2) + diag(r_all(2:3), 1));
r = (r + r') + diag(r_all(rdiag_start_counter:rdiag_end_counter));
...
xi_means = [Mu_L; Mu_S; Mu_C]; %Also had Mu_Nims before
