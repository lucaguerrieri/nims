...
delete empiricaltable.out
diary empiricaltable.out
diary on
...
for country = {'SZ', 'UK', 'CA', 'JP', 'DE', 'CP'}
    ...
    while j <= (t - R)
        [betalarge(:,j) betaintlarge rlarge(:,j)] = regress(y(j:j+R-1,:), [x(j:j+R-1,:)]);

        f_in_roll1    = [f_in_roll1;  rlarge(end,j)^2 ]; 
        f_in_roll2    = [f_in_roll2;  y(j+R-1,:).^2 ];
        f_oos_roll1   = [f_oos_roll1 ;  [x(j+R,:)]*betalarge(:,j)];
    ...
    xlim([1 size(dates,1)])
    print(h, '-dpsc', strcat(char(ylbl),'decomp_gr.eps'))
    hold off
